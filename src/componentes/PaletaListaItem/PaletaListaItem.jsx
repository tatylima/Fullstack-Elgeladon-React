...
function PaletaListaItem({ paleta, quantidadeSelecionada, index, onRemove, onAdd, clickItem, mode }) {
  ...
  const badgeCounter = (canRender) =>
    Boolean(canRender) && (
      ...
      Boolean(canRender) && (<button disabled={mode !== ActionMode.NORMAL} className="Acoes__remover"onClick={(e) => {
         e.stopPropagation();
          onRemove(index);
        }}>remover</button>
    );
    ...
    onClick={() => clickItem(paleta.id)}>
      ...
      <div>
        ...
        <div className="PaletaListaItem__acoes Acoes">
          ...
          <button disabled={mode !== ActionMode.NORMAL}className={`Acoes__adicionar ${ !quantidadeSelecionada && "Acoes__adicionar--preencher" }`}
